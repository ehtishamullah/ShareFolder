...
%% PARAMETERS
numSensors = 4;
baseline = 300; % km total length
offset = 20;    % km off-axis perturbation

% Sensor positions (collinear + slight offset)
sensorPos = [0     0;
             baseline/4   0;
             2*baseline/3 0;
             baseline/2   offset]; % offset sensor
...
%% PLOT GDOP HEATMAP
figure;
imagesc(-500:10:700, -550:10:550, log10(GDOP_map));
set(gca,'YDir','normal');
colorbar;
title('Geometry Quality (GDOP) - Collinear Deployment');
xlabel('Range (km)');
